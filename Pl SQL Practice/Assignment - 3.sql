...
begin
    ...
    if withdraw > amount then
        ...
        select concat('Account Balance Of ',aid,'Is Updated');
    end if;
...
begin
    ...
    if deposit > 0 then
		select concat('Account Balance Of ',aid,'Is Less Than 0');
	else
		update Accounts
		set amount_balance = amount_balance + amount
        where Account_id = aid;
        ...
        select concat('Account Balance Of ',aid,'Is Updated To ',amount);
    end if;
...
begin
    ...
    select amount_balance into receiversAmmount from Accounts where Account_id = receivers;
    ...
    update Accounts
	set amount = sendersAmmount
	where Account_id = receivers; 
...
begin
	...
	declare label text(30) default '';

	declare displayData cursor for 
    select ename from Employee where dnum = 3;
